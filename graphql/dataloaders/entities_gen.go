...
func configureEntityLoaders(loaders *Loaders, orm *gen.Client, collector metrics.Collector) {
	...
	loaders.register("PostTag", newEntityLoader[string, *gen.PostTag]("post_tags", collector, func(ctx context.Context, keys []string) (map[string]*gen.PostTag, error) {
		results := make(map[string]*gen.PostTag, len(keys))
		for _, key := range keys {
			record, err := orm.PostTags().ByID(ctx, key)
			if err != nil {
				return nil, err
			}
			if record != nil {
...
func (l *Loaders) PostTag() *EntityLoader[string, *gen.PostTag] {
	if l == nil {
		return nil
	}
	if loader, ok := l.get("PostTag").(*EntityLoader[string, *gen.PostTag]); ok {
		return loader
	}
	return nil
}
